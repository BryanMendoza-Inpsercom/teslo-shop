...
const FilterSidebar = () => {
  ...
  const currentSize = searchParam.get('sizes')?.split(',') || []; //xs, xl, s ,m
  const currentPrice = searchParam.get('price') || 'any';

  const handleSizeChange = (size: string) => {

    const newSize = currentSize.includes(size)
      ? currentSize.filter(s => s !== size) : [...currentSize, size];

    const filtrado = newSize.filter(item => item !== '');

    searchParam.set('page', '1');
    searchParam.set('sizes', filtrado.join(','));
